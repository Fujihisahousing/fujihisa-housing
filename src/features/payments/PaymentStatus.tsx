  ...
  const displayRows: DisplayRow[] = useMemo(() => {
    return r.rows.map((row) => {
      ...
      if (rec && !view.useUnit) {
        ...
        // 「山田太郎」と「山田　太郎」のように空白の有無だけが違う表記ゆれが多いため、
      ...
      // 記録の無い月。契約者は部屋の現在値から出すが、次のどれかなら出さない（「—」表示）。
      //   ・部屋の状況が「空室」  … レントロールの状況だけ空室に変えて契約者名が残っている
      //                            部屋でも、空室の月に前の入居者の名前を出さないため
      //   ・部屋の状況が「入予」  … 新しい入居者が入る前。部屋に残っているのは前の入居者の
      //                            名前（新しい契約内容は入退去シートの unit_patch にあり、
      //                            入居日が来るまで部屋には入らない）なので出してはいけない
      //   ・退去日の翌月以降で今も空室 … 退去タブを使った部屋の退去後
      // 「停止」（募集停止）は入居したままのことがあるので、ここでは伏せない。
      // 退去月そのものは、その月の家賃を払った人なので「◯◯（旧）」のまま残す。
      const hideUnit = u.status === '空室' || u.status === '入予' || view.vacant
      return {
        unit: u,
