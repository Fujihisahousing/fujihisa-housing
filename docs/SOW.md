...
### 7.7 監査ログ（変更履歴）
- 台帳（transactions）の**作成・変更・削除を `audit_logs` に自動記録**する（トリガ `log_audit`）。
- `detail`(jsonb) に**変更前(old)・変更後(new)の行全体**を保存し、「いつ・誰が・何を」変更したかを追跡できる（`actor`＝auth.uid()、`actor_email`＝記録時点のメール）。
- **削除は物理削除せず `deleted_at` による論理削除**とし、監査ログにも `delete` として残す。台帳・集計（収支表・入金状況）は `deleted_at IS NULL` のみを対象とし、削除済みデータは非表示だが7年保持（税務対応）と整合。
- **編集・削除・履歴閲覧は admin のみ**（UIで制御。`audit_logs` は RLS で admin のみ `select` 可、書込みはトリガ＝security definer 経由）。
- 台帳の各レコードに「編集」「削除」「履歴」ボタンを設け、履歴ボタンから当該レコードの変更履歴（日時・変更者・変更前→変更後の差分）を一覧表示する。
- 拡張余地：同方式のトリガを units / leases 等にも付与すれば、物件・入居情報の変更履歴も同様に追跡できる（Phase2「スタッフ操作ログ」に対応）。
